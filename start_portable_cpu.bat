...
REM LiveFacer portable CPU launcher
REM - Uses only /system for Python, ffmpeg, git and temp/cache
REM - Uses default /system/python/Lib/site-packages
REM
REM Usage:
REM   start_portable_cpu.bat
REM   start_portable_cpu.bat --deps
REM   start_portable_cpu.bat --check
REM   set MAX_MEMORY_GB=8 && start_portable_cpu.bat
REM   set CPU_THREADS=12 && start_portable_cpu.bat
REM   start_portable_cpu.bat [additional run.py args]
...
REM Force default interpreter packages (avoid inheriting custom PYTHONPATH)
set "PYTHONPATH="
...
set "RUN_ARGS=--execution-provider cpu --execution-threads %CPU_THREADS%"
if defined MAX_MEMORY_GB set "RUN_ARGS=%RUN_ARGS% --max-memory %MAX_MEMORY_GB%"

pushd "%CODE_DIR%"
echo [INFO] Launching LiveFacer on CPU...
call :warn_if_cpu_missing
...
:warn_if_cpu_missing
"%PYTHON_EXE%" -c "import onnxruntime as ort,sys; sys.exit(0 if 'CPUExecutionProvider' in ort.get_available_providers() else 1)" >nul 2>&1
if errorlevel 1 (
    echo [WARN] CPUExecutionProvider not detected by onnxruntime.
    echo [WARN] Try reinstalling onnxruntime:
    echo [WARN]   "%PYTHON_EXE%" -m pip install --upgrade onnxruntime
) else (
    echo [OK] CPUExecutionProvider detected.
)
goto :eof
