...
REM LiveFacer portable AMD (DirectML) launcher
REM - Uses only /system for Python, ffmpeg, git and temp/cache
REM - Prioritizes /system/python/Lib/site-packages-dml
REM
REM Usage:
REM   start_portable_amd.bat
REM   start_portable_amd.bat --deps
REM   start_portable_amd.bat --check
REM   set MAX_MEMORY_GB=8 && start_portable_amd.bat
REM   start_portable_amd.bat [additional run.py args]
...
REM DirectML-friendly defaults
...
if exist "%SYSTEM_DIR%\python\Lib\site-packages-dml\" (
    if defined PYTHONPATH (
        set "PYTHONPATH=%SYSTEM_DIR%\python\Lib\site-packages-dml;%PYTHONPATH%"
    ) else (
        set "PYTHONPATH=%SYSTEM_DIR%\python\Lib\site-packages-dml"
    )
)
...
set "RUN_ARGS=--execution-provider dml --execution-threads 1"
if defined MAX_MEMORY_GB set "RUN_ARGS=%RUN_ARGS% --max-memory %MAX_MEMORY_GB%"

pushd "%CODE_DIR%"
echo [INFO] Launching LiveFacer with DirectML...
call :warn_if_dml_missing
...
:warn_if_dml_missing
"%PYTHON_EXE%" -c "import onnxruntime as ort,sys; sys.exit(0 if 'DmlExecutionProvider' in ort.get_available_providers() else 1)" >nul 2>&1
if errorlevel 1 (
    echo [WARN] DmlExecutionProvider not detected by onnxruntime.
    echo [WARN] Ensure portable folder exists:
    echo [WARN]   "%SYSTEM_DIR%\python\Lib\site-packages-dml"
    echo [WARN] Or install manually:
    echo [WARN]   "%PYTHON_EXE%" -m pip install --upgrade onnxruntime-directml
) else (
    echo [OK] DmlExecutionProvider detected.
)
goto :eof
